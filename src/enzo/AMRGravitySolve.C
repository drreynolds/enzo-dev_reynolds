...
//   amrsolve_params->set_parameter("dump_x", "true");
  ...
  // share boundary values within each level
  ...
  for (int thislevel=level_coarse; thislevel<=level_fine; thislevel++) {
    ...
    // create an array (Grids) of all the grids on this level
    HierarchyEntry **Grids;
    int NumberOfGrids = GenerateGridArray(LevelArray, thislevel, &Grids);
#ifdef FAST_SIB
    SiblingGridList *SiblingList = SiblingGridListStorage[thislevel];
#endif
  ...
  // each level, using interpolation from parent grid (only for refined levels)
  for (int thislevel=level_coarse; thislevel<=level_fine; thislevel++)
    if (thislevel > 0) 
      for (Temp=LevelArray[thislevel]; Temp; Temp=Temp->NextGridThisLevel) 
